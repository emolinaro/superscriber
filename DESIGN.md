...
2. Media transport directly above the transcript: Play/Pause toggle, jump-back, playback rate, and current segment label. Audio recordings add a decoded-wave progress bar: the drawn wave is the real seek surface (click/drag plus arrow-key/Home/End seeking), with clickable per-segment markers, an active-segment band, and a timecode readout. When the runtime cannot decode audio the transport restores the native audio controls as the fallback; video always keeps native controls. A numbered segment rail below seeks the media and surfaces that segment inline in the transcript, focusing its review affordance. Active-segment attribution is half-open (startMs <= t < endMs) so marker and rail seeks land on the target segment. When playable media is unavailable, the transport shell retains the prior pinned layout and keeps its availability notice visible while the transcript scrolls underneath. On media (audio or video) casefiles the player and the transcript share one bounded column (media-casefile-video-visible): the desktop containment stacks the transport above a transcript scrollport behind a hard five-card floor - at the reference desktop viewport (1280x800) at least five segment cards sit fully visible below the player in both review and approver views, with a clear column gap between player and segments. The video picture is always visible as the full player look (column-wide letterboxed frame, native controls including fullscreen expand); the video frame is the column's flexible surface and shrinks toward its own floor first - at the 1280x800 reference viewport its column-wide letterboxed frame lands at the explicit 96 px floor - before the transcript floor ever moves. When even the floors cannot fit in a shorter window, including action-mode layouts below the reference height, the whole column scrolls and the dock flows away instead of squeezing the transcript under pinned chrome. The audio transport keeps its compact wave surface within the same stack. Media-casefile chrome densifies to fund the bar: slim case header, single-row revision-summary and speaker toolbar, compact cards with an inline confidence note, and a two-line editing textarea that stays vertically resizable. Below 1100 px the app bar, emergency banner, and case header flow away on casefile pages, leaving the transport as the single top-pinned chrome. The segment rail collapses below 1100 px and at desktop heights through 920 px; transcript timestamps and wave markers retain direct segment seeking. Where it renders, the rail is an independent horizontal scrollport that keeps the same active segment the transcript centers: the active chip stays centered-ish in the rail view as playback advances and after every explicit seek. The rail runs the transcript's non-fighting contract on its own axis - a wheel or touch gesture on the rail pauses only the rail's follow until the active chip intersects the rail view again or an explicit seek re-engages it immediately, and rail gestures never pause the transcript's vertical follow (nor the reverse).
3. Transcript area: 96 px timestamp gutter with playback buttons, 128 px speaker field, flexible text, confidence as subordinate text. A non-active segment button seeks to that segment and plays; the active segment button pauses playback or resumes from its unchanged paused position. Editable only in draft state; pending and approved revisions render immutable text. Editable drafts add a speaker toolbar above the segments listing each unique speaker with its segment count and the batch **Rename speaker...** entry point. Unsaved local edits disable that entry point with an explanation until the reviewer saves or discards them. Phone safety mode removes the toolbar with the other governed editing controls. The active playback row combines a visual marker with `aria-current`, and playback centers the active segment in the visible transcript scrollport. Manual wheel, touch, or page-scroll key gestures pause follow until the next active segment intersects the visible scrollport; any explicit media seek re-engages it immediately, including transcript timestamps, rail chips, wave markers, and progress or native controls. The nested transcript scrollport never traps boundary gestures: wheel and touch input chain into the whole media column at its edges so the surrounding player and casefile chrome stay reachable. Segments the viewed revision changed relative to its in-casefile parent revision carry an `Edited vs vN` marker.
...
- 1100 px and above: full work ledger; the casefile is a bounded viewport-height shell whose case header and pinned action bar frame one transcript scrollport, with the transport pinned inside that scrollport on media-less layouts; media casefiles instead stack the always-visible player (video frame elastic between floor and a quarter-viewport cap, slim audio wave) above a five-card-floor transcript scrollport with a clear gap, and an over-short column scrolls as a whole; an open governance drawer shares the row at 70/30, and the segment rail collapses at heights through 920 px
