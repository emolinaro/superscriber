...
- **Rename speaker (batch)** - only while the current revision is a draft, under the same reviewer save authority as an edit. One command relabels every segment attributed to the source speaker in a single superseding draft revision; renaming onto an existing speaker merges both names onto it. The dialog shows the exact batch summary (moved count plus merge-target count) before commit, the command ignores any client-computed segment payload and derives the batch from the stored draft, and the transaction records an attributed `revision.speakers_renamed` audit event, so the pre-rename wording stays recoverable through the revision lineage.
