...
This file is the design record and the visual and interaction source of truth for the product as it stands on `main`, which carries work toward the next release - see the `[Unreleased]` section of `CHANGELOG.md`; the version source is `VERSION`. It records both how the product looks and how its governed workflow behaves; code is authoritative for anything beyond what is written here.
...
4. **Deliberate governance.** Batch speaker rename, submit, withdraw, approve, request changes, reopen, export, assignment removal, and admin action-mode entry get explicit confirmations proportional to their consequence.
...
- **Save draft** - only while the current revision is a draft; supersedes the prior draft and creates the next numbered draft with complete segment content. Saves and submits follow a patch protocol: the browser submits only the changed text and speaker fields per segment, and the server merges them into the canonical skeleton of the loaded draft revision. Segment count and order always come from the stored draft, every patch id must address an existing segment, and identity, timing, and worker-owned metadata are copied from the stored segment so a patch can never set them. The save and submit audit events record the patch (segment ids and changed fields). New system drafts and reviewer save/submit writes trim every speaker label and require 1-80 characters.
- **Rename speaker (batch)** - only while the current revision is a draft, under the same reviewer save authority as an edit. The source must exist in the stored draft, and the trimmed, required target must differ from it and stay within the 80-character speaker-label limit. One command relabels every segment attributed to the exact source label in a single superseding draft revision; renaming onto an exact existing label merges both names onto it. Before commit, the dialog shows the moved count and, for a merge, the existing target count. The server derives the batch from the stored draft rather than accepting a client-computed segment payload, so a successful command writes the confirmed counts. The transaction records an attributed `revision.speakers_renamed` audit event, so the pre-rename wording stays recoverable through the revision lineage.
...
Admin base access is read-only oversight: every inbox row, every current casefile, transcript, policy, provenance, assignments, revision history, decisions, and audit - plus ingest, account creation, and assignment management as native admin duties. Oversight cannot edit, save, rename speakers, submit, withdraw, approve, request changes, reopen, or export. Under action mode the admin performs those governed operations on ANY casefile regardless of uploader or assignment (captain ruling, admin ledger access): edits, speaker renames, and submissions in reviewer mode; withdrawal of any pending revision with a known submitter in reviewer mode; decisions, reopens, and exports in approver mode. Administration already covers assign/unassign, permanent recording purge, archived-revision recovery, ledger reset, policy, and account roles on every ledger item; every one of those controls attributes the acting admin in the ledger it touches. The one deliberate boundary is in-flight upload-session byte transfer (chunk append/finalize), which stays owner-bound because the bytes and resume token live in the uploader's browser; the recording rows those sessions create are fully admin-visible and admin-managed from ingest verification onward.
...
2. Media transport directly above the transcript: Play/Pause toggle, jump-back, playback rate, and current segment label. Audio recordings add a decoded-wave progress bar: the drawn wave is the real seek surface (click/drag plus arrow-key/Home/End seeking), with clickable per-segment markers, an active-segment band, and a timecode readout. When the runtime cannot decode audio the transport restores the native audio controls as the fallback; video always keeps native controls. Inside the bounded desktop casefile the video frame renders at its original pre-pinned-zone size - the 30vh cap the player-first layout carried before the pinned-zone work (captain ruling 2026-08-19, superseding the same-day 96px band). To keep the transcript budget honest under that frame, video casefiles reclaim vertical room from chrome the pinned contract does not depend on (shell padding, page margin, header density, transport padding, rail chip padding, and the summary row) so the transcript viewport below the pinned zone holds several full cards at every desktop height; audio casefiles are untouched by that budget and render exactly the pinned-zone layout they already had. When the live revision state is changes_requested, a compact Changes requested banner pins at the top of the casefile main column - title, version, attribution, and timestamp always visible, with the full note in its own internal scrollport (the governance decision rows keep the note whole and unclamped - only this top-of-casefile banner is compact). On short desktop windows the banner is budgeted into the pinned zone the same way the video frame is: shell padding, header density, transport padding, wave height, and the summary row reclaim room so the banner never pushes the transcript viewport below its several-card floor and the bounded page never clips (captain ruling; e2e/media-casefile-rest-state.spec.ts proves the 1280x800 geometry with a near-max note). A numbered segment rail below seeks the media and surfaces that segment inline in the transcript, focusing its review affordance. Active-segment attribution is half-open (startMs <= t < endMs) so marker and rail seeks land on the target segment. On desktop (1100 px and above) the casefile page is a bounded viewport-height shell with a pinned zone: the case header, the media box, the transport, and the segment rail never scroll - the window itself carries no scroll range on these pages, so no browser-window scroll can move them. Below 1100 px the app bar, emergency banner, and case header flow away on casefile pages, leaving the transport as the single top-pinned chrome. The segment rail collapses below 1100 px and at desktop heights through 920 px; transcript timestamps and wave markers retain direct segment seeking. Where it renders, the rail is an independent horizontal scrollport that keeps the same active segment the transcript centers: the active chip stays centered-ish in the rail view as playback advances and after every explicit seek. The rail runs the transcript's non-fighting contract on its own axis - a wheel or touch gesture on the rail pauses only the rail's follow until the active chip intersects the rail view again or an explicit seek re-engages it immediately, and rail gestures never pause the transcript's vertical follow (nor the reverse).
3. Transcript area: below the pinned zone the transcript segments list owns the ONLY vertical scrollport on the desktop casefile (the changes-requested banner's compact internal note scrollport in the pinned zone above is the sole exception) - every segment renders its full text as its own card (no clamp, no truncation, no artificial runways; the editor hugs its content) and the full list scrolls inside that viewport. A non-active segment button seeks to that segment and plays; the active segment button pauses playback or resumes from its unchanged paused position. Editable only in draft state; pending and approved revisions render immutable text. Editable drafts add a speaker toolbar above the segments listing each unique speaker with its segment count and the batch **Rename speaker...** entry point. Unsaved local edits disable that entry point with an explanation until the reviewer saves or discards them. Phone safety mode removes the toolbar with the other governed editing controls. The active playback row combines a visual marker with `aria-current`. Playback follow stays dormant at the player-led rest state and activates on the first playback tick or an accepted explicit seek - a rejected seek (for example while media is unavailable) never centers a stale row, and the active segment's play/pause toggle is not a seek. Once active, follow centers interior active segments in the transcript viewport (several context cards stay visible around the playing line; at the canonical 1280x800 desktop window audio casefiles show middle-2 through middle+2 fully visible, and video casefiles reach that same geometry once the window is tall enough for it, per the captain's ruling (b) that centering applies only where geometry physically allows) while the first and final segments anchor honestly at the viewport's block edges. Manual wheel, touch, or page-scroll key gestures pause follow until the next active segment intersects the viewport again; any accepted explicit media seek re-engages it immediately, including transcript timestamps, rail chips, wave markers, and progress or native controls. The viewport keeps default overscroll behavior so wheel/touch gestures chain outward at its edges instead of trapping. Segments the viewed revision changed relative to its in-casefile parent revision carry an `Edited vs vN` marker.
4. Pinned state action bar showing only state-valid commands for the current principal, including the unsaved-state indicator. For admin oversight the bar additionally pins the governed destructive Delete recording control as the last button, so permanent deletion never scrolls away on any layout; phone safety mode withholds it with the rest of the governed row and names the tablet-or-desktop requirement.
...
Administration has secondary navigation for Accounts, Assignments, Policy, and Data discipline; the selected section is the page's `h1` and only its task is shown. Accounts supports search, a compact create-account dialog, an inline role dropdown on every row, and a Reset password control whose governed behavior is owned by [`docs/operators/password-reset.md`](./docs/operators/password-reset.md). Selecting a role other than the persisted role reveals a required 10-500 character Change reason field plus explicit Save role and Cancel actions. Administrators may change any account, including their own, but an active administrator cannot be demoted when no other active administrator remains. The designated break-glass administrator cannot be demoted until the designation moves, and active assignments whose recorded role conflicts with the requested role block the change with a link to the filtered assignment ledger.
...
On the casefile, admin oversight additionally carries a pinned Delete recording control in the state action bar (demo-recording-remove): permanent deletion of the recording and its whole casefile behind a typed-title confirmation (server-rechecked). The purge removes revisions, decisions, assignments, jobs, audit rows, and the media blob, leaves exactly one `recording.deleted` security record, and snapshots all removed rows to `data/ledger-snapshots/` before deletion. The casefile's Revisions governance tab is a full version history: every revision row shows state, timestamps, and summary; archived rows deep-link to a read-only snapshot; an inline `Diff vs active` reveals segment-level differences; and administrators recover archived content through the [governed recovery command](#revision-and-decision-commands).

## Interaction And Copy Rules

- Prefer procedural language: "Session expired. Sign in again to continue."
- Every local account-creation form requires password confirmation. A mismatch is reported live, announced, tied to the confirmation field, marks both password fields invalid, and blocks button- or Enter-key submission; a guarded submit attempt also places the error in the form summary
...
- 1100 px and above: full work ledger; the casefile is a bounded viewport-height shell with a pinned zone (case header, media box, transport, segment rail) above the transcript's own scroll viewport, and the pinned action bar at the bottom; an open governance drawer shares the row at 70/30, and the segment rail collapses at heights through 920 px, where the pinned zone compacts further to hold the transcript's several-card floor
- 768 to 1099 px: full governed actions; transcript first in one column; the casefile app bar, emergency banner, and case header flow away while the compact transport alone pins at the viewport top; governance opens as a side drawer; ledger becomes a semantic list on narrower widths
- Phone safety mode (below 768 px width, or coarse pointer with height below 768 px): read-only casefile and administration, semantic ledger lists, compact viewport-pinned transport, a status-only pinned state bar, governance accordions, supported ingest; the app bar, emergency banner, and case header flow away and no governed action buttons are shown
- The transcript viewport begins within ~500 px of the document top on desktop (the pinned zone above it is compact) and within 500 px on a 390 px phone; there is no page-level horizontal scroll at 320 or 390 px
