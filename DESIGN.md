...
- **Save draft** - only while the current revision is a draft; supersedes the prior draft and creates the next numbered draft with complete segment content. The full segment array is submitted on save; there is no per-keystroke serialization. New system drafts and reviewer save/submit writes trim every speaker label and require 1-80 characters.
