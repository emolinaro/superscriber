...
2. Media transport directly above the transcript: Play/Pause toggle, jump-back, playback rate, and current segment label. Audio recordings add a decoded-wave progress bar: the drawn wave is the real seek surface (click/drag plus arrow-key/Home/End seeking), with clickable per-segment markers, an active-segment band, and a timecode readout. When the runtime cannot decode audio the transport restores the native audio controls as the fallback; video always keeps native controls. A numbered segment rail below seeks the media and surfaces that segment inline in the transcript, focusing its review affordance. Active-segment attribution is half-open (startMs <= t < endMs) so marker and rail seeks land on the target segment. On desktop (1100 px and above) the casefile page window-scrolls with the transport leading the main column: the transcript owns the full viewport so follow-scroll can hold the active segment in the exact middle of the viewport with a working band of context segments around it. Below 1100 px the app bar, emergency banner, and case header flow away on casefile pages at every width, and the transport is the single top-pinned chrome instead. The segment rail collapses below 1100 px; transcript timestamps and wave markers retain direct segment seeking. Where it renders, the rail is an independent horizontal scrollport that keeps the same active segment the transcript centers: the active chip stays centered-ish in the rail view as playback advances and after every accepted explicit seek. The rail runs the transcript's non-fighting contract on its own axis - a wheel or touch gesture on the rail pauses only the rail's follow until the active chip intersects the rail view again or an accepted explicit seek re-engages it immediately, and rail gestures never pause the transcript's vertical follow (nor the reverse).
3. Transcript area: 112 px timestamp gutter with playback buttons, 128 px speaker field, flexible text, confidence as subordinate text. Segment cards never clamp: every segment keeps its full text and natural card height (no line clamp, no truncation, no expand affordance), the draft editor hugs its content, and the transcript is fully scrollable. At the canonical 1280x800 desktop viewport a band of 5-10 context segments stays visible around the active one. A non-active segment button seeks to that segment and plays; the active segment button pauses playback or resumes from its unchanged paused position. Editable only in draft state; pending and approved revisions render immutable text. Editable drafts add a speaker toolbar above the segments listing each unique speaker with its segment count and the batch **Rename speaker...** entry point. Unsaved local edits disable that entry point with an explanation until the reviewer saves or discards them. Phone safety mode removes the toolbar with the other governed editing controls. The active playback row combines a visual marker with `aria-current`, and playback centers the active segment in the middle of the viewport on both axes (symmetric scroll-padding on desktop makes the centered position the exact viewport middle; hold that while the track moves, including across short-to-long segment transitions). Follow stays dormant on mount so the player-led rest state remains at the top, and activates only after the track moves: the first playback tick or an accepted explicit seek. A rejected seek, including one attempted while media is unavailable, never centers a stale row. Manual wheel, touch, or page-scroll key gestures then pause follow until the next active segment intersects the viewport again; an accepted explicit media seek re-engages it immediately, including transcript timestamps, rail chips, wave markers, and progress or native controls. Segments the viewed revision changed relative to its in-casefile parent revision carry an `Edited vs vN` marker.
...
- 1100 px and above: full work ledger; the casefile window-scrolls as one document - case header, transport, and transcript flow in order, the state action bar stays pinned at the bottom, follow-scroll centers the active segment on the viewport middle with a 5-10 segment context band, an open governance drawer shares the row at 70/30, and the segment rail stays available at every desktop height
- 768 to 1099 px: full governed actions; transcript first in one column; the casefile app bar, emergency banner, and case header flow away while the compact transport pins at the viewport top; governance opens as a side drawer; ledger becomes a semantic list on narrower widths
