...
- 1100 px and above: full work ledger; the casefile is a bounded viewport-height shell whose case header and pinned action bar frame one transcript scrollport, with the transport pinned inside that scrollport on media-less layouts; media casefiles instead dock the transport above a minimum-height transcript scrollport (the collapsed-by-default video picture expands to a capped dock), and an over-short column scrolls as a whole; an open governance drawer shares the row at 70/30, and the segment rail collapses at heights through 920 px
