...
- Casefile media transport with an always-visible player: video casefiles keep the full video player with native controls and fullscreen expand, audio casefiles keep the slim decoded-waveform transport, and at the reference desktop viewport at least five transcript segment cards stay fully visible below the player in review and approver views; transcript segments are click-to-play and the decoded waveform is the audio seek surface. See [The player](./docs/USER-GUIDE.md#the-player) for reviewer controls and [Casefile review](./DESIGN.md#casefile-review) for the precise layout and follow-scroll contract
