...
Superscriber is a self-contained governed transcription appliance for sensitive audio and video. Release history lives in [CHANGELOG.md](./CHANGELOG.md); the current version source is the `VERSION` file.
...
- Decoded-waveform media transport on the casefile: the drawn wave is the real seek surface (click, drag, and keyboard seeking) with per-segment markers, an active-segment band, and a timecode readout. Transcript segments are click-to-play: clicking a non-active segment seeks and plays it, clicking the currently playing segment pauses in place, and clicking it again resumes from the unchanged paused position - with Space parity on the transport toggle. On desktop the casefile window-scrolls and playback holds the active segment in the exact middle of the viewport with a band of 5-10 context segments around it - segment cards never clamp, every segment keeps its full text, and the transcript scrolls end to end; below 1100px the transport stays pinned at the viewport top instead. Follow-scroll stays dormant on mount until the first playback tick or an accepted explicit seek, yields to manual scrolling once active, and never centers a stale row after a rejected or unavailable-media seek. Native controls remain the fallback for video and undecodable audio
