...
- `e2e/support/appliance.ts` `uploadFixture` only works for admin uploads: uploader-role ingest redirects to `/workspace?notice=...` by design (`ingest-flow.tsx`), so uploader-owned fixtures resolve the recording id from the ledger instead (pattern in `e2e/admin-ledger-access.spec.ts` `uploadAsUploader`).
