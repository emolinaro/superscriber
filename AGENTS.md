...
- For host-local e2e iteration, `npm run start` silently 404s static chunks (build uses `output: standalone`); serve with `node .next/standalone/server.js` after copying `.next/static` and `public` into `.next/standalone/`, then point `PLAYWRIGHT_BASE_URL` at it.
