...
- For host-local e2e iteration, `npm run start` silently 404s static chunks (build uses `output: standalone`); serve with `node .next/standalone/server.js` after copying `.next/static` and `public` into `.next/standalone/`, then point `PLAYWRIGHT_BASE_URL` at it. Upload-dependent specs also need the Python worker running beside it: share `SUPERSCRIBER_ENGINE_SHARED_SECRET`, point `SUPERSCRIBER_DB_PATH`/`SUPERSCRIBER_UPLOAD_TMP_DIR` at a `.tmp/e2e-data.*` dir (the e2e helpers discover runtime roots there), and set `SUPERSCRIBER_TRANSCRIBE_MODEL=missing-e2e-model SUPERSCRIBER_TRANSCRIBE_OFFLINE=1 SUPERSCRIBER_TRANSCRIBE_ALLOW_STUB_FALLBACK=1` on both processes.
- Known pre-existing e2e failure on main: the first `e2e/oidc.spec.ts` dual-login test fails with `error=OAuthSignin` in the container lane (tracked separately in the superscriber-e2e-ci-fix lane); not caused by and not gated on appearance work.
