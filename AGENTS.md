...
- Parallel e2e lanes on one host are supported: set `SUPERSCRIBER_E2E_PORT`, `SUPERSCRIBER_E2E_OIDC_PORT`, `SUPERSCRIBER_E2E_CONTAINER_NAME`, and `SUPERSCRIBER_E2E_IMAGE` to unique values per lane; lanes that share one worktree must also set `SUPERSCRIBER_E2E_OIDC_DIR` uniquely, otherwise each lane's `start` rewrites the OIDC config directory bind-mounted into the other lane's app container.
